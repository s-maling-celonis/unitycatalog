...
3. Optional host entry for OAuth (browser / curl from host):
...
| OAuth (HTTP) | `http://dev.dev.celonis.cloud:9010` |
| Spark → UC URI | `http://unitycatalog:8080` (Docker network) |

Override with `UC_SERVER_URL`, `UC_OAUTH_BASE_URL`, `UC_ADMIN_TOKEN`, etc.
