 ...
 * mirroring loadTable). `INSERT OVERWRITE DIRECTORY` always requests
 ...
 * applies this rule as an idempotent early fallback for that path.
 *
 ...
 * are left untouched. Ambient `fs.s3a.*` (or other cloud) keys do not skip vending. Duplicated
 * `option.fs.*` entries carry the same UC path identity.
 *
 ...
 * interfere with ambient credentials.
 */
