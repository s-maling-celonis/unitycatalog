...
/**
 * Spark 4.0 lacks the Spark 4.2 v2 view API (`RelationCatalog` / `ViewCatalog` / `View`), so
 * views cannot be created, listed as views, replaced, renamed, or dropped through the connector on
 * this version. Plain SQL views are still readable: they are surfaced on the table listing and
