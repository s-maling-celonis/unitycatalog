 ...
 * plain SQL views are surfaced on the table listing and resolved from their SQL text via a V1 VIEW
 * `CatalogTable`. With [[UCSparkSessionExtensions]], CREATE / SHOW / DROP VIEW are routed through
 * UC REST; replacement and rename remain unsupported. Metric and materialized views stay inert.
 */
